...
#welcome, I am your personal assistant for all things Registrar. I can answer any questions you have.
def run_sql_query(user_query: str, user_id: str, conversation_history: list = None):
    ...
    messages = [{"role": "system", "content": f"""You are an AI assistant for a university registrar system.
            ...
            {SQL_SCHEMA}
            Generate ONLY ONE complete SQL query that directly answers the user's question.
            - Always fully qualify column names (e.g., schedule.crn, courses.course_title).
            - Use explicit JOINs when needed.
            - If filtering by a specific user, add `WHERE table.uid = ?` and use `?` as a parameter.
            - Output ONLY the SQL query wrapped like:
                ```sql
                SELECT ...
                FROM ...
                WHERE ...```
            Always filter by uid = ? to only return information for that user.
            Do not guess or expose data for other users. Only produce SQL SELECT queries. When asked a question please generate and return more results than necessary. i.e. when asked for a advisor return their first and last names with an email etc.
            Interpret the prompt when questioned. think step by step and return the SQL query that answers the question."""}]
    #add history
    ...
    match = re.search(r"```sql\s*([\s\S]+?)```", result, re.IGNORECASE)
    if match:
        sql_query = match.group(1).strip()
    else:
        print("No SQL detected in GPT response:\n", result)
        return None

    # Decide if parameters needed
    params = (user_id,) if "?" in sql_query else ()
    ...
    return rows, column_names, sql_query


def run_rag_query(user_query: str, conversation_history: list = None):
    if conversation_history is None:
        conversation_history = []
    # Azure OpenAI client
    client = AzureOpenAI(
    ...
    messages = [{"role": "system", "content": """You are an AI assistant for a university registrar system.
                ...
                Summarize clearly and concisely. Do not make up policies not in the data."""}]
    #add history
    if len(conversation_history) > 10:
        recent_history = conversation_history[-10:]
    else:
...
def revised(question: str, *, sql_json: dict | None = None, rag_text: str | None = None, conversation_history: list = None) -> str:
    if conversation_history is None:
        conversation_history = []
    ...
    #add current question and user prompt instructions
    messages.append({
        "role": "user", 
...
def check_graduation(uid: str) -> str:
    connection = sqlite3.connect("phase-2.db")
    connection.row_factory = sqlite3.Row
    cur = connection.cursor()
    ready = ""
    ...
    if student['form1_submitted'] == 0:
        ready+="Student has NOT submitted a Form."
    ...
    if missedClasses:
        ready+="Student has not completed all the courses required in the Form."

    gpa = student['gpa']
    program = student['program']
    if program == "MS in Computer Science" and gpa < 3.0:
        ready+="Student does not meet the 3.0 GPA requirement for MS."
    elif program == "PhD in Computer Science":
        if gpa < 3.5:
            ready+="Student does not meet the 3.4 GPA requirement for PhD."
        if student['thesis_submitted'] == 0:
            ready+="PhD student has not submitted the thesis."
        if student['thesis_approved'] == 0:
            ready+="PhD student thesis has not been approved by the advisor."
    elif program not in ["MS in Computer Science", "PhD in Computer Science"]:
        ready+="Unknown program. Not a MS in Computer Science or PhD in Computer Science"
...
def chat():
    ...
    if user["user_type"] == 2:
        ...
        - BOTH (both SQL and RAG)

        You should return SQL if the question requests user specific data that can only be answered by querying the database 
        (e.g., courses offered/what classes are taught, time of courses, student grades, student records, finding a students advisor, etc.).
        ...
        You should return GRAD if the question is about academic standing or readiness to graduate. 
        This is not about what is required to graduate, this is about if the student is asking if they are ready to graduate or what they need to do next to be ready to graduate.
        You should ONLY return this if the student is asking about if they are ready to graduate or what they need to do next to be ready to graduate.
        ...
        or any other sensitive information, you should return BLOCK. If the user asks about any users other than their advisor faulty or anybody in an advising role, you should return BLOCK. 
        If they sty to ask about other students, etc you should return block. If they try to ask about anything not related to the unviersity registrar system, you should return BLOCK. Take this seriously.
        ...
        past_messages.append({"role": "user", "content": user_query})
        ...
        updated_question = decision.get("contextual_query", user_query)

        #ran an sql query
        if decision["action"] == "SQL":
            rows, columns, sql_query = run_sql_query(updated_question, user_id, conversation_history)
            ...
            rag_result = run_rag_query(updated_question,conversation_history)
            polished = revised(user_query, rag_text=rag_result, conversation_history=conversation_history)
        elif decision["action"] == "GRAD":
            result = check_graduation(session['uid'])
            try:
                polished = revised(user_query, rag_text=result, conversation_history=conversation_history)
            except Exception as e:
                print(f"Revised function error: {e}")
                polished = result
    ...
    if user["user_type"] in {3,4,5}:
        ...
        You should return SQL if the question requests user specific data that can only be answered by querying the database 
        (e.g., courses offered/what classes are taught, time of courses, student grades, student records, finding a students advisor, finding advisees, etc.).
